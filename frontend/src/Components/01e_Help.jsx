...
const faqs = [
  ...
  { q: "How can recruiters view resumes?", a: "Recruiters can access resumes of all applicants directly from the Applications section of their dashboard. No email exchange needed." },
...
function getBotReply(input) {
  ...
  const match = faqs.find(f =>
    ...
    f.a.toLowerCase().includes(lower.split(" ").slice(0, 3).join(" "))
  );
  ...
  if (lower.includes("apply") || lower.includes("application")) return faqs[6].a;
  if (lower.includes("password") || lower.includes("reset")) return faqs[9].a;
  if (lower.includes("free") || lower.includes("cost") || lower.includes("price")) return faqs[10].a;
  if (lower.includes("recruiter") || lower.includes("post") || lower.includes("job")) return faqs[4].a;
  if (lower.includes("delete") || lower.includes("account")) return faqs[11].a;
  return null;
}
    ...
    setTimeout(() => {
      ...
      if (reply) {
        ...
        setMessages(prev => [...prev, {
          from: "bot",
          text: "I don't have an answer for that in my knowledge base.",
          contact: true
        }]);
      }
    }, 900);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.hero}>
        ...
        <p className={styles.heroSub}>
          Ask our support assistant any question about HIREON.
          If it can't help, it will connect you directly to our team.
        </p>
      </div>
          ...
          <div className={styles.chatMessages}>
            {messages.map((m, i) => (
              <div key={i} className={m.from === "user" ? styles.msgUser : styles.msgBot}>
                {m.from === "bot" && (
                  <div className={styles.botAvatar}>H</div>
                )}
