...
export const HireonJobsTab = () => {
  ...
  const loadData = () => {
    ...
    // Build set of job IDs this candidate has already applied to
    const appliedJobIds = new Set(
      applications
        .filter(a => a.candidateId === cand.email)
    ...
    const matched = posted.filter(job => {
      // ── KEY CHANGE: exclude jobs already applied to ──
      if (appliedJobIds.has(String(job.id))) return false;

      if (!candAll.length) return true;
      const jobSkills = (job.skills || "")
        .split(/[,]+/)
        .map(s => s.trim().toLowerCase())
        .filter(Boolean);
      if (!jobSkills.length) return true;
  ...
  // After applying, reload so the job disappears from the list
  const onApplied = () => { loadData(); };
  ...
  if (jobs.length === 0) {
    const cand = JSON.parse(localStorage.getItem("hireon_candidate")) || {};
    const applications = JSON.parse(localStorage.getItem("hireon_applications")) || [];
    const hasApplied = applications.some(a => a.candidateId === cand.email);

    return (
      <div className={styles.emptyState}>
        <div className={styles.emptyIcon}>🎯</div>
        <p className={styles.emptyTitle}>
          {hasApplied ? "No new matching jobs" : "No matching HIREON jobs yet"}
        </p>
        <p className={styles.emptySub}>
          {hasApplied
            ? "You've applied to all matching jobs. Check Application Status to track your submissions, or check back later for new postings."
            : !cand.skills?.length
    ...
    <div className={styles.wrap}>
      <div className={styles.headerRow}>
        <p className={styles.matchCount}>
          <span>{jobs.length}</span> job{jobs.length !== 1 ? "s" : ""} matched to your skills
        </p>
      </div>
              ...
              <div className={styles.jobCardLeft}>
                ...
                <div>
                  ...
                  <div className={styles.jobTags}>
                    ...
                    {job.deadline   && (
                      <span className={expired ? styles.tagRed : styles.tagYellow}>
