  ...
  return (
    <div className={`${styles.card} ${decision === "Accepted" ? styles.cardAccepted : decision === "Rejected" ? styles.cardRejected : ""}`}>
      ...
      {atsScore > 0 && (
        <div className={styles.scoreRow}>
          <div className={styles.scoreTop}>
            ...
            <span className={styles.scoreNum} style={{ color: scoreColor(atsScore) }}>{atsScore}%</span>
          </div>
          <div className={styles.scoreTrack}>
            <div className={styles.scoreFill} style={{ width: `${atsScore}%`, background: scoreColor(atsScore) }} />
          </div>
        </div>
      )}
      ...
      <div className={styles.actions}>
        <button className={styles.resumeBtn} onClick={() => onViewResume(app)}>
          📄 Resume
        </button>
        {!decision
...
export const CandidatesApplied = () => {
  ...
  useEffect(() => {
    ...
    const enriched = myApps.map(a => ({
      ...a,
      resumeB64: a.candidateEmail === candProfile.email
        ? candProfile.resumeB64
        : (a.resumeB64 || null),
    }));
    ...
    <div className={styles.page}>
      ...
      <nav className={styles.navbar}>
        <div className={styles.navLeft}>
          ...
          <button className={styles.dashBtn} onClick={() => navigate("/Recruiter/06_MainRec")}>
            Dashboard
          </button>
        </div>
        <div className={styles.navRight}>
          <span className={styles.navName}>{recruiter.name || "Recruiter"}</span>
          <button className={styles.signOutBtn} onClick={() => navigate("/Recruiter/02_LoginRec")}>
            Sign out
          </button>
        </div>
      </nav>
                  ...
                  return (
                    ...
                    >
                      <div className={styles.jobTabRow}>
                        ...
                        <span className={styles.jobTabCount}>
                          {appsForJob(job.id).filter(a => !decisions[String(job.id)]?.[String(a.id)]).length} pending
                        </span>
                      </div>
